...
# Network Upload Speed Monitor
# Shows current upload speed with dynamic indicators

cache_file="/tmp/network_up_cache"
...
# Get current bytes transmitted
current_tx=$(cat /sys/class/net/$interface/statistics/tx_bytes 2>/dev/null)
...
if [ -f "$cache_file" ]; then
    ...
    prev_tx=$(echo "$prev_data" | cut -d: -f1)
    prev_time=$(echo "$prev_data" | cut -d: -f2)
    
    time_diff=$((current_time - prev_time))
    
    if [ "$time_diff" -gt 0 ]; then
        bytes_diff=$((current_tx - prev_tx))
...
echo "$current_tx:$current_time" > "$cache_file"