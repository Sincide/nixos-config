...
# Detailed Memory Usage Monitor
# Shows used/total with cached and available info
...
mem_cached_gb=$(echo "scale=1; $mem_cached / 1024 / 1024" | bc 2>/dev/null)
...
# Output with dynamic icon based on usage percentage and cache info
if [ "$mem_percent" -ge 90 ]; then
    echo "💀 ${mem_used_gb}/${mem_total_gb}GB (${mem_percent}%)"
