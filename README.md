...
```
nixos-config/
├── flake.nix              # Main flake configuration (includes claude-code)
...
- For unfree package errors (claude-code): Check that `nixpkgs.config.allowUnfree = true` is in both `hosts/common.nix` and `home.nix`
